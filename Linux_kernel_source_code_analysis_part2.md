...
其中存在几个重要的方法：
1. add_to_swap_cache，将空内存页面写入用户空间，置于可交换的空间中，契机就是在页表项发现已失效的时候，需要换回内存。
2. alloc_page，分配多个物理页面，__alloc_page作为实际的内存页面分配函数
3. rmqueue尝试从页面管理区中分配若干连续的内存页面
4. expand用于拆分大块页面，并插入链表
